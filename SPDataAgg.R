...
close(channel)
#saveRDS(SDdata, file = "FullIGFS_AbunDist.rds")
levels(as.factor(IGFSdata$fldPrimeStation))
levels(as.factor(IGFSdata$Species))
levels(as.factor(IGFSdata$Yr)) 

year2003<- IGFSdata[IGFSdata$Yr=="2003",]
sp2003 <- levels(year2003$Species)

AggData <- data.frame(yr.list = c(levels(as.factor(IGFSdata$Yr))),
                      stn.list = c(levels(as.factor(IGFSdata$fldPrimeStation)))#,
                      
)
SPDataAgg <- aggregate(x=AggData[c(specieslist)],
  by = c(IGFSdata$fldPrimeStation,IGFSdata$Yr,IGFSdata$Species), FUN = sum)
  years <- levels(as.factor(IGFSdata$Yr)) 
