...
## REMEMBER TO CHECK THAT THE database is up-to-date
## (if it is not, then run the update job first!)
...
# IMPORTANT - Change this value to extract either IAMS or IGFS data
surveyToUse <- 'IGFS'
folderToUse <- paste0("./updatedData/",surveyToUse,"/")

abundanceQuery <- paste0("SELECT [Cruise],[Yr],[Haul],isnull([fldPrimeStation],'H:'+cast([Haul] as varchar)) as fldPrimeStation,[Time_Min],[LonDec],[LatDec],[DoorSpread],[Dist_Nmi],[AreaKmSq],[ObsVal],[RECT],[Stratum],[ICESCODE],[Validity],[Species],[Sex],[fldGearCode],[fldGearDescription],[CatchKg],[RaisedNo],[LoadDate] FROM [dbo].[SurveyAbundanceAndDistribution] WHERE Cruise like '",surveyToUse,"%'")
data1<- sqlQuery(channel,abundanceQuery, stringsAsFactors = TRUE)
...
catchRateQuery <- paste0("SELECT [Cruise],[Haul],isnull([Prime_Stn],'H:'+cast([Haul] as varchar)) as Prime_Stn,[fldICESRectangle],[fldStratum],[Area],[ICESCODE],[fldValidityCode],[Lon],[Lat],[Species],[fldCommonName],[fldScientificName],[fldSex],[GearDescription],[Catch_Kg],[Kg_Per_Hr],[LoadDate] FROM [dbo].[SurveyCatchRate] WHERE Cruise like '",surveyToUse,"%'")
dat<- sqlQuery(channel,catchRateQuery, stringsAsFactors = TRUE)
...
lengthQuery <- paste0("SELECT * FROM [dbo].[SurveyLenghtData] WHERE fldCruiseName like '",surveyToUse,"%'")
LengthData<- sqlQuery(channel,lengthQuery, stringsAsFactors = TRUE)
...
LengthWeightAgeQuery <- paste0("SELECT * FROM [dbo].[SurveyLenghtWeightAge] WHERE fldCruiseName like '",surveyToUse,"%'")
LengthWeightAge<- sqlQuery(channel,LengthWeightAgeQuery, stringsAsFactors = TRUE)
...
stationQuery <- paste0("SELECT [FID],[fldCruiseName], isnull([fldPrimeStation],'H:'+cast(fldCruiseStationNumber as varchar)) as fldPrimeStation,[fldGearCode],[fldShotDepth],[fldHaulDepth],[fldCruiseStationNumber],[HaulCnt],[Gear_Type],[SweepLngt],[fldICESRectangle],[fldValidityCode],[fldDateTimeShot],[TowDurationMin],[fldShotLatDecimalDegrees],[fldShotLonDecimalDegrees],[fldHaulLatDecimalDegrees],[fldHaulLonDecimalDegrees],[fldStratum],[DoorSpread],[fldWingSpread],[fldHeadlineHeight],[Dist_Nmi],[Dist_Km],[AreaKmSq],[ObsVal],[SweepAngle],[Warp],[Heave_m],[Pitch_deg],[Roll_deg],[WindSpeed_Kts],[fldGearDamage],[fldStationComment],[LoadDate]  FROM [dbo].[SurveyMI_Survey_Station_Data] WHERE fldcruisename like '",surveyToUse,"%'")
stn<- sqlQuery(channel,stationQuery, stringsAsFactors = TRUE)
...
saveRDS(stn,paste0(folderToUse,"stn.RDS"))
...
## Getting ready NEP data (commented out because we'll just use the database now)
...
#######################For mapdata###########################
 ...
 saveRDS(mapdata,paste0(folderToUse,"mapdata.RDS"))
...
saveRDS(sp_data_gp,paste0(folderToUse,"sp_data_gp.RDS"))
...
saveRDS(data1,paste0(folderToUse,"data1.RDS"))
#saveRDS(dat,"updatedData/dat.RDS")
saveRDS(dat,paste0(folderToUse,"dat.RDS"))
#saveRDS(dat1,"updatedData/dat1.RDS")
saveRDS(dat1,paste0(folderToUse,"dat1.RDS"))
#saveRDS(LengthWeightAge,"updatedData/LengthWeightAge.RDS")
saveRDS(LengthWeightAge,paste0(folderToUse,"LengthWeightAge.RDS"))
#saveRDS(LengthData,"updatedData/LengthData.RDS")
saveRDS(LengthData,paste0(folderToUse,"LengthData.RDS"))
#saveRDS(mapdataS,"updatedData/mapdataS.RDS")
saveRDS(mapdataS,paste0(folderToUse,"mapdataS.RDS"))
#saveRDS(TotalNumbersMap,"updatedData/TotalNumbersMap.RDS")
saveRDS(TotalNumbersMap,paste0(folderToUse,"TotalNumbersMap.RDS"))
#saveRDS(AdultNumbersMap,"updatedData/AdultNumbersMap.RDS")
saveRDS(AdultNumbersMap,paste0(folderToUse,"AdultNumbersMap.RDS"))
#saveRDS(JuvNumbersMap,"updatedData/JuvNumbersMap.RDS")
saveRDS(JuvNumbersMap,paste0(folderToUse,"JuvNumbersMap.RDS"))
...
unlink(paste0(folderToUse,"coeff_L_A.csv"))
write.csv(all_data,paste0(folderToUse,"coeff_L_A.csv"),row.names = F)
