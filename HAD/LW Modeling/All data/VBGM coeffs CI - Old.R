...
predictedCI=confint(fitTyp)
pvLower = predictedCI["Linf", 1]*(1-exp(-predictedCI["K", 1]*(4-predictedCI["t0",1])))
pvUpper = predictedCI["Linf", 2]*(1-exp(-predictedCI["K", 2]*(4-predictedCI["t0",2])))

#CI of life parameters
LinfCI=quantile(ests[,1], c(0.025, 0.975))
KCI=quantile(ests[,2], c(0.025, 0.975))
t0CI=quantile(ests[,3], c(0.025, 0.975))

pvLower = LinfCI[[1]]*(1-exp(-KCI[[1]]*(4-t0CI[[1]])))
pvUpper = LinfCI[[2]]*(1-exp(-KCI[[2]]*(4-t0CI[[2]])))


#write ests to rds and read back in
write_rds(ests, "testingests.RDS")
testRDS=readRDS("testingests.RDS")
...
dfSampleCI = LWA_fish %>% group_by(Year) %>%
  #do(fitSample = try(nls(length ~ vbTyp(age, Linf, K, t0), data=., start=svTyp))) %>%
  do(fitSampleCI = try(confint(nls(length ~ vbTyp(age, Linf, K, t0), data=., start=svTyp))))
  


dfSampleBoot = LWA_fish %>% group_by(Year) %>%
  do(fitSample = try(nls(length ~ vbTyp(age, Linf, K, t0), data=., start=svTyp))) %>%
  fitBoot= nlsBoot(fitSample)
...
myfunc <- function(data, indices) {
  data <- data[indices,]
  try(nls(length ~ vbTyp(age, Linf, K, t0), data=data, start=svTyp))
  #mean(data$age)
  }

library(boot)
bootresults <- LWA_fish %>% 
  group_by(Year) %>%
...
boot_test=LWA_fish %>% 
  group_by(Year) %>%
  fittest=nls(length ~ vbTyp(age, Linf, K, t0), data=., start=svTyp)

dfSample = LWA_fish %>% nest(-Year) %>%
  bootstrap(1000) %>%
  #mean=mean(.$age)
  do(fitSample = try(nls(length ~ vbTyp(age, Linf, K, t0), data=., start=svTyp)))
...
boot_test= LWA_fish %>% 
  nest(-Year) %>% 
  #mutate(cors_boot = map(data, ~ bootstrap(., 10) %>% 
  #                         do(tidy(cor.test(.$age,.$length))))) %>% 
