function (start.list, data, binding, maxiter.em = 1000, reltol = 1e-08, 
  ...
  for (i in 1:maxiter.em) {
    ...
    if (i > 2) {
      if (abs(ollike[i] - ollike[i - 1]) < abs(ollike[i - 
        ...
        female.se <- c(par.se[binding[, "female"]], par.se[max(binding) + 
                                                             1])
        male.pars <- c(oll.fit$par[binding[, "male"]], 
                       -oll.fit$par[max(binding) + 1])
        male.se <- c(par.se[binding[, "male"]], par.se[max(binding) + 
                                                         1])
...
<bytecode: 0x0000000028c615e0>
  <environment: namespace:lhmixr>
  > 