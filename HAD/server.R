...
shinyServer(function(input, output) {
  ...
  cat=reactive({
    dplyr::filter(dat1, Cruise %in% paste0('IGFS', input$slideryear))
  })
  ...
  x1 <- reactive({LengthWeightAge$age[which(LengthWeightAge$length== (input$length+0.5))]})
  output$agerange <- reactive({
    if (length(x1())<3) {
      paste("No individuals of this length recorded",sep="")
    }else{ 
      paste(min(x1(), na.rm=TRUE),"to", max(x1(), na.rm=TRUE), "years,", "n = ",length(x1()), sep=" ")
    }
  })
  ...
  output$latab=renderUI({
    if(dim(LengthWeightAgeSpA1())[1]==0){
      ...
      list(
        plotlyOutput("laplot"),
        fluidRow(column(3,h3("Life Parameters"),
                        tableOutput("coeff_table"))
                 #,
                 #column(3,h3("Predictions using VBGM"),
                #        numericInput("age", "Fish Age:", value=0, min = 0, max = 10),
                ...
                #        tableOutput("predage")),
                # column(3, h3("Predictions using mode:"),
                #        tags$b("Age range observed*:"), h4(textOutput("agerange")),
